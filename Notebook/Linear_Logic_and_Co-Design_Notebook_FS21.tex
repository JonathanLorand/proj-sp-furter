...
\subsubsection{A Optimization Procedure}
...
Formulated as a transaction between us an a provider $\Theta$ this becomes the following: For each upper set $U_R$, the provider has a single deal. If you choose to accept the deal corresponding to $U_R$, the provider selects a $l_R \in U_R$. You must the give the provider any $r \in l_R$. In return you get to choose freely a $l_F \in \Theta(U_R)$. The provider then gives you some $f \in l_F$ according to his discretion. 
...
We should consider a chain of processes 
$$\cdots \xslashedrightarrow{\Phi_i} \low(\mc{R}_i) \xslashedrightarrow{\Phi_{i+1}} \low(\mc{R}_{i+1}) \xslashedrightarrow{\Phi_{i+2}} \cdots.$$
The resources we can guarantee at $\low(\mc{R}_i)$ depends on what choice of transformation we make for $\Phi_i$.  We can lift this chain to a chain of functors
$$\cdots \stackrel{\hat\Phi_i}{\rightarrow} \upper(\low(\mc{R}_i)) \stackrel{\hat\Phi_{i+1}}{\rightarrow} \upper(\low(\mc{R}_{i+1})) \stackrel{\hat\Phi_{i+2}}{\rightarrow} \cdots.$$
